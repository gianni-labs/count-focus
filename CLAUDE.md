...
- `main.go` — arg parsing entrypoint (`run`). Handles `--help`/`-h` and `--version`/`-v`; `parseArgs` walks the args into an `options{duration, title}`. Exactly one duration source is required: a bare `<duration>`, `--preset`/`-p <name>`, or `--until`/`-u <HH:MM>`; `--title`/`-t` is optional and can appear in any position. `run` then calls `RunCountdown(duration, title)`. `version` is a package var overridable via `-ldflags -X main.version=...` (see Distribution); local builds default to `"dev"`.
...
- `duration_test.go`, `format_test.go`, `progressbar_test.go`, `until_test.go`, `preset_test.go`, `main_test.go` — table-driven tests for the pure logic and arg parsing above.
