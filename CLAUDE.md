...
- `main.go` — arg parsing entrypoint (`run`). Handles `--help`/`-h` and `--version`/`-v`; `parseArgs` resolves either a bare duration or `--preset`/`-p <name>` to a `time.Duration`, then `run` calls `RunCountdown`. `version` is a package var overridable via `-ldflags -X main.version=...` (see Distribution below); local builds default to `"dev"`.
